...
inline void bitonic_cas_ascending(__m256& a, __m256& b)
{
...
inline void bitonic_cas_descending(__m256& a, __m256& b)
{
    __m256 min_val = _mm256_min_ps(a, b);
    __m256 max_val = _mm256_max_ps(a, b);
    a = max_val;
...
inline __m256 reverse_ps(__m256 v)
{
...
// ============================================================================
inline void bitonic_sort_8(__m256& data)
{
    // Stage 1: pairs
...
// ============================================================================
inline void bitonic_merge_16(__m256& lo, __m256& hi)
{
...
private:
    ...
    static inline int32_t unpack_index(uint64_t kv)
    {
        return static_cast<int32_t>(kv & 0xFFFFFFFF);
    }
...
public:
    ...
    inline void partial_sort(const float* values, std::vector<int>& pos, int n_elmts, int K)
    {
        ...
        else
        {
            ...
            std::sort(kv_pairs, kv_pairs + n_elmts, [](uint64_t a, uint64_t b) {
                return (a >> 32) < (b >> 32);
            });
        }
...
private:
    ...
    {
        if ((kv_pairs[i] >> 32) > (kv_pairs[j] >> 32))
            std::swap(kv_pairs[i], kv_pairs[j]);
    }
    ...
    inline void bitonic_sort_32(int offset, bool ascending)
    {
        ...
        for (int i = offset; i < offset + 32; i += 2)
            if (ascending == ((i / 2) % 2 == 0)) cas_asc(i, i + 1); else cas_desc(i, i + 1);
                ...
                for (int dist = size / 2; dist >= 1; dist /= 2)
                {
                    ...
                    else
                    {
                        ...
                        for (int k = 0; k < size; k += 2 * dist)
                        {
                            ...
                            {
                                if (asc) cas_asc(i + k + j, i + k + j + dist);
                                else cas_desc(i + k + j, i + k + j + dist);
                            }
                        }
                    }
                }
    ...
    inline void bitonic_merge_prune_64(int off_a, int off_b, bool out_ascending)
    {
        ...
        for (int dist = 16; dist >= 1; dist /= 2) {
             ...
             else
             {
                 for (int k = 0; k < 32; k += 2 * dist) {
                     for (int j = 0; j < dist; j++) {
                         if (out_ascending) cas_asc(offset + k + j, offset + k + j + dist);
                         else cas_desc(offset + k + j, offset + k + j + dist);
                     }
                 }
             }
        }
