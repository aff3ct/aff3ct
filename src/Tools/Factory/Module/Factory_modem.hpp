...
struct Factory_modem : public Factory
{
	struct parameters
	{
		...
		int         N_mod      = -1;        // frame size at the output of the modulator
		...
		int         N_fil      = -1;        // frame size at the output of the filter
...
private:
	...
	static module::Modem<B,R,Q>* _build(const parameters &params);

	template <typename B = int, typename R = float, typename Q = R>
	static module::Modem<B,R,Q>* _build_scma(const parameters &params);
};
}
}
