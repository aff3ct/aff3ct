...
{
	struct flip_and_check_parameters
	{
		bool enable              = false;
		int  q                   = 10;
		int  ite_min             = 3;
	...
	static Flip_and_check<B,Q>* build(const flip_and_check_parameters& params, module::CRC<B> &crc, const int n_frames = 1);

	static void build_args(Arguments_reader::arg_map &req_args, Arguments_reader::arg_map &opt_args);
	static void store_args(const Arguments_reader& ar, flip_and_check_parameters &params,
	                       const int size, const int n_ite, const int start_crc_check_ite = 2);
	static void group_args(Arguments_reader::arg_grp& ar);

	static void header(Header::params_list& head_fnc, const flip_and_check_parameters& params);
