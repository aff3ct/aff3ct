...
template <typename B>
void Encoder_RSC4_sys<B>
::encode(const mipp::vector<B>& U_K, mipp::vector<B>& X_N)
{
	assert(U_K.size() == (unsigned) (K * this->n_frames));
	assert(X_N.size() == (unsigned) ((N + 2*4) * this->n_frames));

	if (buffered_encoding)
		for (auto f = 0; f < this->n_frames; f++)
		{
			std::copy   (U_K.data() + f*K, U_K.data() + f*K +K, X_N.data() + f*2*(K+4)); // sys
			frame_encode(U_K.data() + f*K, X_N.data() + f*2*(K+4) +K, 1, true);          // par + tail bits
		}
	else
		for (auto f = 0; f < this->n_frames; f++)
			frame_encode(U_K.data() + f*K, X_N.data() + f*2*(K+4));
}
...
{
	assert(par.size() == (unsigned) ((K +2*4) * this->n_frames));

	// par bits: [par | tail bit sys | tail bits par]
	for (auto f = 0 ; f < this->n_frames ; f++)
		frame_encode(U_K.data() + f*K, par.data() + f*(K+2*4), 1, true);
}
...
template <typename B>
mipp::vector<mipp::vector<int>> Encoder_RSC4_sys<B>
::get_trellis()
{
	...
	for (auto i = 0; i < 16; i++)
	{
		...
		// std::cout << "state = " << i       << ", ";
		// std::cout << "sys   = " << bit_sys << ", ";
		// std::cout << "par   = " << bit_par << std::endl;
...
::frame_encode(const B* U_K, B* X_N, const int stride, const bool only_parity)
{
	...
	auto state = 0; // initial (and final) state 0 0 0 0
	...
	// tail bits for initialization conditions (state of data "state" have to be 0 0 0 0)
	for (auto i = 0; i < 4; i++)
	{
		...
		if (!only_parity)
		{
			...
			X_N[j+4] = bit_sys; // systematic transmission of the bit
	...
	assert(state == 0);
	if (!only_parity)
		assert(j == (N + 2*4) * stride);
	else
	{
		j += 4 * stride;
		assert(j == (K + 2*4) * stride);
	}
}
