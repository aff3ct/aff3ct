...
void Simulation_main::build_args(arg_map &req_args, arg_map &opt_args)
{
...
void Simulation_main::store_args(const tools::Arguments_reader& ar, parameters &params)
{
...
void Simulation_main::group_args(arg_grp& ar)
{
	ar.push_back({"sim", "Simulation parameter(s)"});
	ar.push_back({"cde", "Code parameter(s)"      });
}

void Simulation_main::header(params_list& head_sim, const parameters& params)
{
