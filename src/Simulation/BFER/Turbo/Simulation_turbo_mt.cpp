...
#include "Simulation_turbo_mt.hpp"

template <typename B, typename R, typename Q, typename QD>
Simulation_turbo_mt<B,R,Q,QD>
...
::~Simulation_turbo_mt()
{
	...
	for (auto tid = 0; tid < n_threads; tid++) if (sub_encoder[tid] != nullptr) delete sub_encoder[tid];
...
template <typename B, typename R, typename Q, typename QD>
void Simulation_turbo_mt<B,R,Q,QD>
::launch()
{
	this->snr_loop();
}
...
::snr_loop()
{
	...
	for (auto snr = simu_params.snr_min; snr <= simu_params.snr_max; snr += simu_params.snr_step)
	{
		...
		for (auto tid = 1; tid < n_threads; tid++)
			threads[tid -1] = std::thread(Simulation_turbo_mt<B,R,Q,QD>::simulation_Monte_Carlo, this, snr, tid);

		// launch the master thread
		Simulation_turbo_mt<B,R,Q,QD>::simulation_Monte_Carlo(this, snr, 0);
		...
		{
			analyzer_red->reduce();
			Simulation_turbo_mt<B,R,Q,QD>::time_reduction(this);
...
template <typename B, typename R, typename Q, typename QD>
void Simulation_turbo_mt<B,R,Q,QD>
::simulation_Monte_Carlo(Simulation_turbo_mt<B,R,Q,QD> *simu, const R snr, const int tid)
{
	// allocate and build all the communication chain for simulate: encoding -> add noise -> decoding...
	Simulation_turbo_mt<B,R,Q,QD>::build_communication_chain(simu, snr, tid);
	...
	Simulation_turbo_mt<B,R,Q,QD>::simulation_loop(simu, tid);
}

template <typename B, typename R, typename Q, typename QD>
void Simulation_turbo_mt<B,R,Q,QD>
::build_communication_chain(Simulation_turbo_mt<B,R,Q,QD> *simu, const R& snr, const int tid)
{
	...
	if (simu->sub_encoder[tid] != nullptr) delete simu->sub_encoder[tid];
	...
	if (simu->sf         [tid] != nullptr) delete simu->sf         [tid];
	if (simu->decoder    [tid] != nullptr) delete simu->decoder    [tid];
	if (simu->siso       [tid] != nullptr) delete simu->siso       [tid];
	...
	simu->encoder[tid] = Factory_encoder_turbo<B>::build(simu->code_params, simu->enco_params, simu->interleaver, 
	                                                     simu->sub_encoder[tid], simu->sub_encoder[tid], n_fra);
	...
	simu->decoder[tid] = Factory_decoder_turbo<B,Q>::build(simu->code_params, simu->enco_params, simu->chan_params, 
	                                                       ...
	                                                       simu->siso[tid], simu->siso[tid], 
	                                                       simu->sf[tid], simu->crc[tid]);
...
template <typename B, typename R, typename Q, typename QD>
void Simulation_turbo_mt<B,R,Q,QD>
::simulation_loop(Simulation_turbo_mt<B,R,Q,QD> *simu, const int tid)
{
	         ...
	         (steady_clock::now() - simu->t_snr) < simu->simu_params.stop_time))
	{
		    ...
		    (steady_clock::now() - simu->t_simu) >= simu->simu_params.display_freq)
		{
			...
			Simulation_turbo_mt<B,R,Q,QD>::time_reduction(simu);
...
template <typename B, typename R, typename Q, typename QD>
void Simulation_turbo_mt<B,R,Q,QD>
::time_reduction(Simulation_turbo_mt<B,R,Q,QD> *simu)
{
...
#ifdef MULTI_PREC
template class Simulation_turbo_mt<B_8,R_8,Q_8,QD_8>;
template class Simulation_turbo_mt<B_16,R_16,Q_16,QD_16>;
template class Simulation_turbo_mt<B_32,R_32,Q_32,QD_32>;
template class Simulation_turbo_mt<B_64,R_64,Q_64,QD_64>;
#else
template class Simulation_turbo_mt<B,R,Q,QD>;
#endif
// ==================================================================================== explicit template instantiation