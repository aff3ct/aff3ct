...
public:
	...
	void generate_trellis	  (mipp::vector<int>& trellis_next_state,          mipp::vector<SOUT>& trellis_related_wave_form   );
	void generate_anti_trellis(mipp::vector<int>& anti_trellis_original_state, mipp::vector<SIN>& anti_trellis_input_transition);

	// transition_to_binary :  value = 0 or 1 ; bit 0 is lsb, bit cpm.n_b_per_s-1 is msb
	virtual void generate_mapper(mipp::vector<SIN>& transition_to_binary, mipp::vector<SIN>& binary_to_transition, const std::string& mapping_layout) = 0;
