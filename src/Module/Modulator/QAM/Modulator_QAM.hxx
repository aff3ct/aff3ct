...
::Modulator_QAM(const int N, mipp::vector<R> &H, const int bits_per_symbol, const R sigma, const bool disable_sig2,
...
::demodulate(const mipp::vector<Q>& Y_N1, mipp::vector<Q>& Y_N2)
{
	...
	else
	{
		for (auto n = 0; n < size; n++)// Boucle sur les LLRs
		{
			...
			auto complex_Yk = std::complex<Q>(      Y_N1[2*k], Y_N1[      2*k+1]);
			auto complex_Hk = std::complex<Q>(this->H   [2*k], this->H   [2*k+1]);
