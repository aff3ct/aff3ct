...
template<typename B>
Encoder_turbo_product<B>::Encoder_turbo_product(const Encoder<B>& enc_r,
                                                const Encoder<B>& enc_c,
  ...
  , X_N_i(this->N)
{
        ...
        (!parity_extended && this->N != enc_r.get_N() * enc_c.get_N()))
    {
        ...
        message << "'N' has to be equal to ('enc_r.get_N()' +1) * ('enc_c.get_N()' +1) if parity code extension, "
                << "else equal to 'enc_r.get_N()' * 'enc_c.get_N()'   ('N' = " << this->N
                << ", 'enc_r.get_N()' = " << enc_r.get_N() << ", 'enc_c.get_N()' = " << enc_c.get_N()
    ...
    {
        std::stringstream message;
        message << "'K' has to be equal to 'enc_r.get_K()' * 'enc_c.get_K()' ('K' = " << this->K
                << ", 'enc_r.get_K()' = " << enc_r.get_K() << ", 'enc_c.get_K()' = " << enc_c.get_K() << ").";
...
Encoder_turbo_product<B>::clone() const
{
...
template<typename B>
void
Encoder_turbo_product<B>::_encode(const B* U_K, B* X_N, const size_t frame_id)
{
    ...
    for (int j = 0; j < n_rows_K; j++)
    {
        ...
        {
            this->X_N_i[(j + 1) * n_cols_N - 1] =
              tools::compute_parity(this->X_N_i.data() + j * n_cols_N, n_cols_N - 1);
            X_row_par_off++;
        }
    }
    ...
    for (int j = 0; j < n_cols_N; j++)
    {
        ...
        if (this->parity_extended)
        {
            this->X_N_i[(j + 1) * n_rows_N - 1] =
              tools::compute_parity(this->X_N_i.data() + j * n_rows_N, n_rows_N - 1);
            X_col_par_off++;
        }
...
template<typename B>
void
Encoder_turbo_product<B>::set_n_frames(const size_t n_frames)
{
