#ifdef CHANNEL_GSL

#ifndef CHANNEL_AWGN_GSL_LR_HPP_
#define CHANNEL_AWGN_GSL_LR_HPP_

#include "Channel_AWGN_GSL_LLR.hpp"

namespace aff3ct
{
namespace module
{
template <typename R = float>
class Channel_AWGN_GSL_LR : public Channel_AWGN_GSL_LLR<R>
{
public:
	Channel_AWGN_GSL_LR(const int N, const R& sigma, const int seed = 0, const int n_frames = 1, 
	                    const std::string name = "Channel_AWGN_GSL_LR");

	virtual ~Channel_AWGN_GSL_LR();

	void add_noise(const R *X_N, R *Y_N); using Channel_AWGN_GSL_LLR<R>::add_noise;
};
}
}

#endif // CHANNEL_AWGN_GSL_LR_HPP_

#endif
