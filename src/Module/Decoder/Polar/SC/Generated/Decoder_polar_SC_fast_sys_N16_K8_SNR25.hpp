...
namespace module
{
static const char Decoder_polar_SC_fast_sys_fb_16_8_25[16] = {
...
{
public:
	Decoder_polar_SC_fast_sys_N16_K8_SNR25(const int& K, const int& N, const mipp::vector<B>& frozen_bits, const int n_frames = 1)
	: Decoder_polar_SC_fast_sys<B, R, API_polar>(K, N, frozen_bits, n_frames)
	{
		...
		auto i = 0;
		while (i < 16 && Decoder_polar_SC_fast_sys_fb_16_8_25[i] == frozen_bits[i]) i++;
		assert(i == 16);
	}
