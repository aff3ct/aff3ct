...
::_store_fast(mipp::vector<B>& V) const
{
	if (V.size() != (unsigned) this->N)
		throw std::length_error("aff3ct::module::Decoder_polar_SCL_fast_sys: \"V.size()\" has to be equal to "
		                        "\"N\".");
...
::_unpack(mipp::vector<B>& V_N) const
{
	if (V_N.size() != (unsigned) this->N)
		throw std::length_error("aff3ct::module::Decoder_polar_SCL_fast_sys: \"V_N.size()\" has to be equal to "
		                        "\"N\".");
