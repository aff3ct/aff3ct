  ...
  buffered_encoding(enc_params.buffered)
{
	...
	try
	{
		std::shared_ptr<Encoder_RSC_DB<B>> enc(factory::Encoder_RSC_DB::build<B>(enc_params));
	...
	try
	{
		std::shared_ptr<Decoder_SISO_SIHO<B,Q>> decoder_siso_siho(factory::Decoder_RSC_DB::build_siso<B,Q>(dec_params, trellis, this->get_encoder()));
	...
	{
		std::shared_ptr<Decoder_SIHO<B,Q>> dec(factory::Decoder_RSC_DB::build<B,Q>(dec_params, trellis, this->get_encoder()));
