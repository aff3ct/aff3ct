...
// 	auto r_a0_prev = mipp::load<R>(&this->alpha[0][0]);
// 	auto r_a1_prev = mipp::load<R>(&this->alpha[1][0]);
// 	auto r_a2_prev = mipp::load<R>(&this->alpha[2][0]);
// 	auto r_a3_prev = mipp::load<R>(&this->alpha[3][0]);
// 	auto r_a4_prev = mipp::load<R>(&this->alpha[4][0]);
// 	auto r_a5_prev = mipp::load<R>(&this->alpha[5][0]);
// 	auto r_a6_prev = mipp::load<R>(&this->alpha[6][0]);
// 	auto r_a7_prev = mipp::load<R>(&this->alpha[7][0]);
...
// 		mipp::store<R>(&this->alpha[0][i +stride], r_a0);
// 		mipp::store<R>(&this->alpha[1][i +stride], r_a1);
// 		mipp::store<R>(&this->alpha[2][i +stride], r_a2);
// 		mipp::store<R>(&this->alpha[3][i +stride], r_a3);
// 		mipp::store<R>(&this->alpha[4][i +stride], r_a4);
// 		mipp::store<R>(&this->alpha[5][i +stride], r_a5);
// 		mipp::store<R>(&this->alpha[6][i +stride], r_a6);
// 		mipp::store<R>(&this->alpha[7][i +stride], r_a7);
...
::compute_gamma_alpha(const mipp::vector<R> &sys, const mipp::vector<R> &par)
{
	...
	for (auto i = 0; i < (this->K +3) * stride; i += stride)
	{
		...
		const auto r_a0_prev = mipp::load<R>(&this->alpha[0][i]);
		const auto r_a1_prev = mipp::load<R>(&this->alpha[1][i]);
		const auto r_a2_prev = mipp::load<R>(&this->alpha[2][i]);
		const auto r_a3_prev = mipp::load<R>(&this->alpha[3][i]);
		const auto r_a4_prev = mipp::load<R>(&this->alpha[4][i]);
		const auto r_a5_prev = mipp::load<R>(&this->alpha[5][i]);
		const auto r_a6_prev = mipp::load<R>(&this->alpha[6][i]);
		const auto r_a7_prev = mipp::load<R>(&this->alpha[7][i]);
...
::compute_beta_ext(const mipp::vector<R> &sys, mipp::vector<R> &ext)
{
	...
	auto r_b0_prev = mipp::load<R>(&this->alpha[0][0]);
	auto r_b1_prev = mipp::load<R>(&this->alpha[1][0]);
	auto r_b2_prev = mipp::load<R>(&this->alpha[2][0]);
	auto r_b3_prev = mipp::load<R>(&this->alpha[3][0]);
	auto r_b4_prev = mipp::load<R>(&this->alpha[4][0]);
	auto r_b5_prev = mipp::load<R>(&this->alpha[5][0]);
	auto r_b6_prev = mipp::load<R>(&this->alpha[6][0]);
	auto r_b7_prev = mipp::load<R>(&this->alpha[7][0]);

	// compute beta values [trellis backward traversal <-]
	for (unsigned i = (this->K +2) * stride; i >= (this->K +0) * stride; i -= stride)
	{
	...
	for (auto i = (this->K -1) * stride; i >= 0; i -= stride)
	{
		...
		const auto r_sum0_0 = mipp::add<R>(mipp::add<R>(r_a0, r_b0_prev), r_g0); auto r_max0 = r_sum0_0;
		const auto r_sum0_1 = mipp::add<R>(mipp::add<R>(r_a1, r_b4_prev), r_g0);      r_max0 = MAP(r_max0, r_sum0_1);
		const auto r_sum0_2 = mipp::add<R>(mipp::add<R>(r_a2, r_b5_prev), r_g1);      r_max0 = MAP(r_max0, r_sum0_2);
		const auto r_sum0_3 = mipp::add<R>(mipp::add<R>(r_a3, r_b1_prev), r_g1);      r_max0 = MAP(r_max0, r_sum0_3);
		const auto r_sum0_4 = mipp::add<R>(mipp::add<R>(r_a4, r_b2_prev), r_g1);      r_max0 = MAP(r_max0, r_sum0_4);
		const auto r_sum0_5 = mipp::add<R>(mipp::add<R>(r_a5, r_b6_prev), r_g1);      r_max0 = MAP(r_max0, r_sum0_5);
		const auto r_sum0_6 = mipp::add<R>(mipp::add<R>(r_a6, r_b7_prev), r_g0);      r_max0 = MAP(r_max0, r_sum0_6);
		const auto r_sum0_7 = mipp::add<R>(mipp::add<R>(r_a7, r_b3_prev), r_g0);      r_max0 = MAP(r_max0, r_sum0_7);

		const auto r_sum1_0 = mipp::sub<R>(mipp::add<R>(r_a0, r_b4_prev), r_g0); auto r_max1 = r_sum1_0;
		const auto r_sum1_1 = mipp::sub<R>(mipp::add<R>(r_a1, r_b0_prev), r_g0);      r_max1 = MAP(r_max1, r_sum1_1);
		const auto r_sum1_2 = mipp::sub<R>(mipp::add<R>(r_a2, r_b1_prev), r_g1);      r_max1 = MAP(r_max1, r_sum1_2);
		const auto r_sum1_3 = mipp::sub<R>(mipp::add<R>(r_a3, r_b5_prev), r_g1);      r_max1 = MAP(r_max1, r_sum1_3);
		const auto r_sum1_4 = mipp::sub<R>(mipp::add<R>(r_a4, r_b6_prev), r_g1);      r_max1 = MAP(r_max1, r_sum1_4);
		const auto r_sum1_5 = mipp::sub<R>(mipp::add<R>(r_a5, r_b2_prev), r_g1);      r_max1 = MAP(r_max1, r_sum1_5);
		const auto r_sum1_6 = mipp::sub<R>(mipp::add<R>(r_a6, r_b3_prev), r_g0);      r_max1 = MAP(r_max1, r_sum1_6);
		const auto r_sum1_7 = mipp::sub<R>(mipp::add<R>(r_a7, r_b7_prev), r_g0);      r_max1 = MAP(r_max1, r_sum1_7);
